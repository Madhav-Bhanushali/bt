...
# --- Step 6: confirm PR #25603 (Q2_0 __byte_perm extraction) is in the tree --
# Rejected: hand-patched Q2_0 vec-dot (VDR 1->2) was 7% slower AND garbled
# output; do NOT patch the kernel ourselves. The legitimate merged upstream
# optimization "cuda: extract Q2_0 elements via __byte_perm" (#25603) is the
# only accepted kernel change. Warn loudly if it's missing from this checkout.
if grep -q "0x020100FF" ggml/src/ggml-cuda/vecdotq.cuh; then
    echo "OK: PR #25603 (Q2_0 __byte_perm extraction) present in vecdotq.cuh"
else
    echo "WARNING: Q2_0 __byte_perm extraction (PR #25603) NOT found in vecdotq.cuh."
    echo "  This checkout may be stale. Rebuild from fresh origin/master before benchmarking."
fi
